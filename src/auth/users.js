...
users.generateTokenUp =  function (user) {
  // ,{expiresIn:900}
  console.log('sadasdsadasdasd',user);
  console.log('sadasdsadasdasd',roles[user.role]);
  let token = jwt.sign({username: user.username , capabilities: roles[user.role]},{expiresIn:900}, SECRET );
...
users.generateTokenIn =  async function (user) {
  ...
  // ,{expiresIn:900}
  let token = await jwt.sign({username: username , capabilities: capabilities},{expiresIn:900}, SECRET );
  console.log('tokentokentokentokentoken',token);
  console.log('##########################################################');
  return token;
};
